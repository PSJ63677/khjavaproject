...
//		char [] copy = origin;		// 얕은 복사, 원본이 수정되어 버림
		char [] copy = new char[14];	// 깊은 복사의 시작, 반드시 공간을 새로 할당 해줘야함
		for(int i= 0; i < origin.length; i++) {
			if(i < 8) {
				copy[i] = origin[i];
			} else {	
			...
			System.out.print(copy[i]);	// 7까지는 origin 값을 받고 8부터 masking
		}
		System.out.println();
		for(int i = 0; i < origin.length; i++) {
			System.out.print(origin[i]);
